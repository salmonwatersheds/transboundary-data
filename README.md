...
### Spawner surveys
As of January 2026, spawner time series coming from NuSEDS are processed in our all-regions NuSEDS update, and so this code only compiled spawner data from other source (mainly the TTC report). Recommended revisions to the NuSEDS data (see `code/1_clean-nuseds.R`) from our TWG include:

- TATSAMENIE RIVER coho (`POP_ID = 45152`) for 1994 and earlier changed to TATSATUA RIVER (`45154`) and remaining records 1995+ are removed.
- any records of `45151` for 1994 or earlier get changed to `45153` (TATSATUA RIVER river-type sockeye)
